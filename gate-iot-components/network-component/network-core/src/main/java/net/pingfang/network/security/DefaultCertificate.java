...
public class DefaultCertificate implements Certificate {
	...
	public X509Certificate[] getTrustCerts() {
		...
		return Arrays.stream(trustHelper.getTrustMgrs()).filter(X509TrustManager.class::isInstance)
				.map(X509TrustManager.class::cast).map(X509TrustManager::getAcceptedIssuers).flatMap(Stream::of)
