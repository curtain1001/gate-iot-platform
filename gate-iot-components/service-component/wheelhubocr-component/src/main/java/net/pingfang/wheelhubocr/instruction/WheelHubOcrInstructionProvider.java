 ...
 */
@Component
public class WheelHubOcrInstructionProvider implements InstructionProvider {
	@Resource
	InstructionConfigManager instructionConfigManager;

	@Override
	public String getName() {
		return WheelHubOcrProduct.WHEEL_HUB_OCR.getValue();
	}

	@Override
	public List<Instruction> getCommand() {
		List<InsEntity> entities = instructionConfigManager.getInstruction(WheelHubOcrProduct.WHEEL_HUB_OCR);
		if (CollectionUtils.isNotEmpty(entities)) {
			...
			if (CollectionUtils.isNotEmpty(instructions)) {
				instructions.add(new WheelHubOcrInstruction());
				return instructions;
			}
		}
		return Collections.singletonList(new WheelHubOcrInstruction());
	}
}
