 ...
 */
@RestController @RequestMapping("business/device") public class BtpDeviceController extends BaseController {

    @Resource IBtpDeviceService btpDeviceService;
    @Resource IBtpInstrDeviceService instrDeviceService;
    @Resource DefaultDeviceOperatorManager operatorManager;
    @Resource DefaultInstructionManager instructionManager;

    @GetMapping("/product/list") public AjaxResult getDeviceProduct() {
     ...
     * @return 分页数据
     */
    @PreAuthorize("@ss.hasPermi('business:device:list')") @GetMapping("/list") public TableDataInfo list(
            BtpDevice device) {
        startPage();
        LambdaQueryWrapper<BtpDevice> queryWrapper = Wrappers.lambdaQuery();
        queryWrapper
                .like(StringUtils.checkValNotNull(device.getDeviceId()), BtpDevice::getDeviceId, device.getDeviceId());
     ...
     * @return 所有设备
     */
    @GetMapping("/all/{laneId}") public AjaxResult getDevices(@PathVariable("laneId") Long laneId) {
        LambdaQueryWrapper<BtpDevice> queryWrapper = Wrappers.lambdaQuery();
     ...
     * 根据id查询
     */
    @PreAuthorize("@ss.hasPermi('business:device:query')") @GetMapping(value = "/{id}") public AjaxResult load(
     ...
     * 根据设备号查询
     */
    @GetMapping(value = "device-id/{deviceId}") public AjaxResult getDevice(@PathVariable String deviceId) {
        LambdaQueryWrapper<BtpDevice> queryWrapper = Wrappers.lambdaQuery();
     ...
     * 根据设备名查询
     */
    @GetMapping(value = "device-name/{deviceName}") public AjaxResult getLaneName(@PathVariable String deviceName) {
     ...
     * 新增通道信息
     */
    @PreAuthorize("@ss.hasPermi('business:device:add')") @Log(title = "设备管理", businessType = BusinessType.INSERT) @PostMapping public AjaxResult add(
            @Validated @RequestBody BtpDevice device) {
        LambdaQueryWrapper<BtpDevice> wrapper = Wrappers.lambdaQuery();
     ...
     * 修改参数配置
     */
    @PreAuthorize("@ss.hasPermi('business:device:edit')") @Log(title = "设备管理", businessType = BusinessType.UPDATE) @PutMapping public AjaxResult edit(
            @Validated @RequestBody BtpDevice device) {
        LambdaQueryWrapper<BtpDevice> wrapper = Wrappers.lambdaQuery();
        wrapper.eq(BtpDevice::getDeviceName, device.getDeviceName());
     ...
     * 删除通道
     */
    @PreAuthorize("@ss.hasPermi('business:device:remove')") @Log(title = "设备管理", businessType = BusinessType.DELETE) @DeleteMapping("/{ids}") public AjaxResult remove(
     ...
     * 开启设备
     */
    @PreAuthorize("@ss.hasPermi('business:device:open')") @Log(title = "设备管理", businessType = BusinessType.UPDATE) @PutMapping("{id}/{status:(?:on|off)}") public AjaxResult openDevice(
            @Validated @PathVariable Long id, @Validated @PathVariable String status) {
        ...
        if ("on".equals(status)) {
            ...
            DeviceOperator deviceOperator = operatorManager
                    .create(device.getLaneId(), device.getDeviceId(), ProductSupports.getSupport(device.getProduct()));
